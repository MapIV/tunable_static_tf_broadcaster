...
TunableStaticTfBroadcasterNode::TunableStaticTfBroadcasterNode(
  ...
  RCLCPP_DEBUG(
      this->get_logger(),
      "Setting parameters... {rate: %f, header_frame: %s, child_frame: %s}",
      publish_rate_, transform_.header.frame_id.c_str(),
      transform_.child_frame_id.c_str());
...
void TunableStaticTfBroadcasterNode::onTimer() {
  ...
  transform_.header.stamp =
      this->now() + rclcpp::Duration::from_seconds(
                        1 / publish_rate_);  // Set future time to allow
...
SetParametersResult TunableStaticTfBroadcasterNode::onParameter(
  ...
  auto update_param = [&parameters](const std::string& name, double& v) {
    const auto it = std::find_if(parameters.cbegin(), parameters.cend(),
...
RCLCPP_COMPONENTS_REGISTER_NODE(
    tunable_static_tf_broadcaster::TunableStaticTfBroadcasterNode)
